...
function addSection(doc: PDFKit.PDFDocument, title: string, y: number): number {
  ...
  doc.fontSize(SECTION_SIZE).font('Helvetica-Bold').fillColor('#1e293b').text(title, MARGIN, y);
  y += SECTION_SIZE + 4;
...
export function generateResumePdfBuffer(resume: ResumeExportData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    doc.fontSize(HEADING_SIZE + 4).font('Helvetica-Bold').fillColor('#0f172a')
      .text(contact.fullName || 'Resume', MARGIN, y, { align: 'center' });
    y += HEADING_SIZE + 8;

    const contactLine = [contact.email, contact.phone, contact.location].filter(Boolean).join('  |  ');
    if (contactLine) {
      doc.fontSize(9).font('Helvetica').fillColor('#64748b').text(contactLine, MARGIN, y, { align: 'center' });
      y += 16;
    }

    if (resume.summary) {
      y = addSection(doc, 'PROFESSIONAL SUMMARY', y);
      doc.fontSize(FONT_SIZE).font('Helvetica').fillColor('#334155')
    ...
    if (resume.experience && resume.experience.length > 0) {
      ...
      for (const exp of resume.experience) {
        if (y > 720) { doc.addPage(); y = MARGIN; }
        doc.fontSize(FONT_SIZE).font('Helvetica-Bold').fillColor('#1e293b').text(exp.role || '', MARGIN, y);
        const dateStr = [exp.startDate, exp.endDate || 'Present'].filter(Boolean).join(' – ');
        doc.fontSize(9).font('Helvetica').fillColor('#64748b').text(dateStr, 400, y, { align: 'right', width: 145 });
        y += LINE_HEIGHT;
        doc.fontSize(FONT_SIZE - 1).font('Helvetica-Oblique').fillColor('#475569').text(exp.company || '', MARGIN, y);
        y += LINE_HEIGHT;
        for (const bullet of (exp.bullets ?? [])) {
          if (y > 750) { doc.addPage(); y = MARGIN; }
          doc.fontSize(FONT_SIZE - 1).font('Helvetica').fillColor('#334155')
    ...
    if (resume.education && resume.education.length > 0) {
      ...
      for (const edu of resume.education) {
        ...
        doc.fontSize(FONT_SIZE).font('Helvetica-Bold').fillColor('#1e293b').text(degreeField, MARGIN, y);
        const dateStr = [edu.startDate, edu.endDate].filter(Boolean).join(' – ');
        doc.fontSize(9).font('Helvetica').fillColor('#64748b').text(dateStr, 400, y, { align: 'right', width: 145 });
        y += LINE_HEIGHT;
        doc.fontSize(FONT_SIZE - 1).font('Helvetica-Oblique').fillColor('#475569').text(edu.institution || '', MARGIN, y);
        y += LINE_HEIGHT + 4;
      }
    }
    ...
    if (resume.projects && resume.projects.length > 0) {
      ...
      for (const proj of resume.projects) {
        if (y > 720) { doc.addPage(); y = MARGIN; }
        doc.fontSize(FONT_SIZE).font('Helvetica-Bold').fillColor('#1e293b').text(proj.name || '', MARGIN, y);
        y += LINE_HEIGHT;
        doc.fontSize(FONT_SIZE - 1).font('Helvetica').fillColor('#334155')
        ...
        if (proj.tech && proj.tech.length > 0) {
          doc.fontSize(9).font('Helvetica-Oblique').fillColor('#64748b').text(proj.tech.join(', '), MARGIN, y);
          y += 14;
        }
        y += 4;
      }
